...
class _Level2State extends State<Level2> {
  ...
  // Ensure the first number is always larger to avoid negative results
  if (number1 < number2) {
    int temp = number1;
    number1 = number2;
    number2 = temp;
  ...
  String questionTitle = '$number1 - $number2 =';
...
Map<String, bool> _generateOptions(int correctAnswer, int maxNumber) {
  ...
  while (optionsSet.length < 4) {
    int deviation = _random.nextInt(11) - 5;  // +/- 5 range of deviation
    int fakeAnswer = correctAnswer + deviation;

    // Check to ensure plausible range for subtraction to avoid unrealistic negative numbers
    if (fakeAnswer >= 1 && fakeAnswer <= maxNumber) {
